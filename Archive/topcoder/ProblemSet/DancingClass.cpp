...
{  
    ...
    {  
        BigNum mat(1), now(0), c(1);
        for(int i = 0; i < n-1; i++)mat = mat*2;
        for(int i = 1; i <= n-1; i++){
            c = c*(n-i+1)/i;
            if(i*(n-i)>=k){
                now = now+c;
        ...
        if(now > mat)return ("High");
        else if(now > mat-1)return ("Equal");
        else return ("Low");
    }  
...
// BEGIN CUT HERE
	...
	void test_case_5() { int Arg0 = 5; int Arg1 = 6; string Arg2 = "High"; verify_case(5, Arg2, checkOdds(Arg0, Arg1)); }
