...
void pushdown(int k, int l, int r)
{
    ...
    if(tree[k].tag){
        ...
        ll c = tree[k].tag;
        int m = (l + r) / 2;
        tree[k * 2 + 1].bit2 += tree[k * 2 + 1].bit1*c*2 + c*c*(id[m]-id[l]);
        tree[k * 2 + 1].bit1 += c*(id[m]-id[l]);
        tree[k * 2 + 2].bit2 += tree[k * 2 + 2].bit1*c*2 + c*c*(id[r]-id[m]);
        tree[k * 2 + 2].bit1 += c*(id[r]-id[m]);
...
void update(int a, int b, int k, int l, int r, int c)
{
    ...
    if(l >= a && r <= b){
        tree[k].tag += c;
        tree[k].bit2 += tree[k].bit1*c*2 + c*c*(id[r]-id[l]);
        tree[k].bit1 += c*(id[r]-id[l]);
        return;
    }
    int m = (l + r) / 2;
    pushdown(k, l, r);
...
void work()
{
    ...
    for(int i = 0; i < n; i++){
        if(i && evt[i].x > evt[i - 1].x){
            ans += (ll)(evt[i].x - evt[i - 1].x) * tree[0].bit2;
